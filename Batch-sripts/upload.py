...
def create_threedimodel(
    ...
    if threedimodel:
        ...
        if not threedimodel.is_valid:
            print(f"Failed to sucessfully process threedimodel with id {threedimodel.id}, the revision is not valid")
    else:
        print('Failed to create threedimodel. Check the number of models that have been created in previous revisions')
...
if __name__ == "__main__":
    ...
    deelgebieden_fn = "Your path to your submodels"
    with open(deelgebieden_fn, "r") as deelgebieden:
        for i in deelgebieden:    
            a = i.strip()
            tags = ["Your tags", a]
            sqlite_path = 'Path_to_schematisation/'+a+'/schematisation/'+a+'.sqlite'
            schematisation_name = "Name of your schematisation" + a
            ...
            print ('Finished uploading model' + a)
