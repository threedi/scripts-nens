...
srs = "EPSG:28992" #fill in your own projection
raster_resolution = 2
...
THREEDI_API = ThreediApi(config=CONFIG, version='v3')


def download(scenario_name, out_dir, download_max_waterdepth=True, download_raw_results=True):
...
if __name__ == "__main__":
    for bui in ['XXmm', 'XXmm']:
        ...
        for schematisation in SCHEMATISATIONS:
            ...
            out_dir = os.path.join("Your_directory_path" + str(nummah))
            try:
                os.makedirs(out_dir)
            except OSError:
                pass
